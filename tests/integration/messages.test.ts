...
// tests/integration/messages.test.ts
// FIXED: Creates own conversation + participants in beforeAll
// FIXED: Cleans up all created data in afterAll
// FIXED: Proper user seeding to satisfy FK constraints
...
// // ── Dynamic user IDs (seeded in beforeAll) ─────────────────────────────────────
...
//   // ── Seed test users (CRITICAL FOR FK CONSTRAINTS) ───────────────────────────
//   const users = await sql`
//     INSERT INTO users (id, username, email, name)
//     VALUES
//       (gen_random_uuid(), 'student_test', 'student@test.com', 'Student'),
//       (gen_random_uuid(), 'instructor_test', 'instructor@test.com', 'Instructor'),
//       (gen_random_uuid(), 'admin_test', 'admin@test.com', 'Admin')
//     RETURNING id
//   `

//   // ── Assign generated DB IDs ────────────────────────────────────────────────
//   STUDENT_ID = users[0].id
//   INSTRUCTOR_ID = users[1].id
//   ADMIN_ID = users[2].id

//   // ── Generate tokens using REAL DB users ────────────────────────────────────
//   studentToken = app.jwt.sign({
//     userId: STUDENT_ID,
...
//     it('returns 200 when sender deletes own message', async () => {
//       if (!createdMessageId) {
//         console.warn('⚠️ Skipping delete test — no createdMessageId')
//         return
//       }
