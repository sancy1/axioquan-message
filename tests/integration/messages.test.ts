...
beforeAll(async () => {
  ...
  // ── Create a fresh test conversation directly in DB ──────────────────────────
  // Using a unique title so we can identify and clean it up
...
describe('Messages API — Integration', () => {
  ...
  describe('GET /api/conversations/:conversationId/messages', () => {
    ...
    it('returns messages with sender details', async () => {
      ...
      const body = JSON.parse(response.body)
      expect(body.success).toBe(true)
      // Empty array is valid — conversation was just created
      expect(Array.isArray(body.data)).toBe(true)
      if (body.data.length > 0) {
        const message = body.data[0]
        expect(message).toHaveProperty('sender')
  ...
  describe('DELETE /api/messages/:id', () => {
    it('returns 200 when sender deletes own message', async () => {
      // Skip gracefully if message creation test failed
      if (!createdMessageId) {
