...
beforeAll(async () => {
  ...
  // ── SAFE: create ONLY conversation (no user insert) ───────
  const convRows = await sql`
    INSERT INTO conversations (type, title, created_by)
    VALUES ('direct', 'Integration test conversation', ${STUDENT_ID})
    RETURNING id
  ...
  testConversationId = convRows[0].id

  // ── participants (safe, no user creation) ─────────────────
  await sql`
    INSERT INTO conversation_participants (conversation_id, user_id, role)
    VALUES
      (${testConversationId}, ${STUDENT_ID}, 'student'),
